...
error_reporting(E_ALL); // Show errors for debugging
...
if (isset($_POST['submit'])) {
    ...
    $othername    = isset($_POST['othername']) ? mysqli_real_escape_string($con, $_POST['othername']) : '';
    $matricNo     = isset($_POST['matricNo']) ? mysqli_real_escape_string($con, $_POST['matricNo']) : '';
    ...
    // Default password = lowercase of last name, hashed with MD5
    $defaultPassword = md5(strtolower($lastname));

    // Check if student exists
    $query = mysqli_query($con, "SELECT * FROM tblstudent WHERE matricNo='$matricNo'");
    if (mysqli_num_rows($query) > 0) {
        $alertStyle = "alert alert-danger";
        $statusMsg  = "Student with this Matric Number already exists!";
    } else {
        // Insert new student
        $sql = "INSERT INTO tblstudent 
            (firstName, lastName, otherName, matricNo, password, levelId, facultyId, departmentId, sessionId, dateCreated)
            VALUES 
            ('$firstname', '$lastname', '$othername', '$matricNo', '$defaultPassword', '$levelId', '$facultyId', '$departmentId', '$sessionId', '$dateCreated')";

        if (mysqli_query($con, $sql)) {
            $alertStyle = "alert alert-success";
            $statusMsg  = "Student added successfully! Default password is their last name.";
        } else {
            $alertStyle = "alert alert-danger";
            $statusMsg  = "Error occurred: " . mysqli_error($con);
        }
    }
}
?>
